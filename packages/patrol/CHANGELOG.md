...
- **Android `PatrolJUnitRunner`:** On **`ConnectException` / `SocketException`** to **`PatrolAppService` (:8082)** during **`listDartTests`** or **`runDartTest`**, relaunch the app with **`FLAG_ACTIVITY_CLEAR_TASK`**, reset the app-ready latch, **`waitForPatrolAppService`**, and recreate the client (up to 3 attempts). Replaces **`ConditionVariable`** readiness with a resettable **`CountDownLatch`** so recovery works on **API 28** (no **`ConditionVariable.close()`**).
